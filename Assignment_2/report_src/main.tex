...
\section*{\textbf{Resubmission}}
We submitted the assignment before the deadline on 3/3/2021. We had used pandas only to generate the tables in a human-readable format and not for any of the solutions. It was mentioned that pandas could be used in one of the comments on the assignment (see image below). Hence, we were under the impression that it is allowed to have a dependency on pandas package for generating the tables. We have now removed the usages of pandas data frames and request you to waive the late submission penalty.
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.65]{Pandas.png}
\end{figure}
\cleardoublepage
